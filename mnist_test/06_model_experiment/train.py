...
def train_model():
    ...
    for epoch in range(CONFIG["EPOCHS"]):
        ...
        for batch_idx, (x, m, t) in enumerate(train_loader):
            ...
            # Loss Morph: MSE -> Gaussian NLL
            # loss_morph = F.mse_loss(m_hat, m, reduction='sum') * 100
            
            # NLL = 0.5 * sum( logvar + (m - mu)^2 / exp(logvar) )
            # 안정성을 위해 상수항 무시하고 스케일 조정
            # m은 (B, 12), m_mu는 (B, 12)
            nll = 0.5 * torch.sum(m_logvar + (m - m_mu)**2 / m_logvar.exp())
            loss_morph = nll # * 10 (스케일 조정 필요 시)
